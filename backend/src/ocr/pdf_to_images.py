...
def pdf_to_images(pdf_path: Path, out_dir: Path) -> List[Path]:
    ...
    # Use 'w' if you want to overwrite the log every time the script runs
        ...
        # e.stderr is None when stderr=subprocess.STDOUT is used (stderr is merged into stdout/log file)
        err = e.stderr.decode(errors="ignore").strip() if e.stderr else "unknown error (see pdftoppm.log)"
        raise RuntimeError(
            f"pdftoppm failed while processing PDF: {err}"
        )
